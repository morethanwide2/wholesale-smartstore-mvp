...
class MasterProduct(TimestampMixin, Base):
    ...
    manufacturer: Mapped[str | None] = mapped_column(String(100))
    origin: Mapped[str | None] = mapped_column(String(100))
    search_tags: Mapped[list | None] = mapped_column(JSONB)
    notice_info_json: Mapped[dict | None] = mapped_column(JSONB)
    supply_price: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    shipping_fee: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    ...
    validation_status: Mapped[str] = mapped_column(String(30), nullable=False, default="not_checked", index=True)
    validation_issues_json: Mapped[list | None] = mapped_column(JSONB)
